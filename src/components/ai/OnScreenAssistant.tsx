...
export function OnScreenAssistant({ isFullScreen: isEmbeddedFullScreen = false, inputRef: externalInputRef }: OnScreenAssistantProps) {
  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      const title = language === 'ne' ? 'कृषि मित्र कुराकानी' : language === 'hi' ? 'कृषि मित्र बातचीत' : 'Krishi Mitra Chat';
...
<body>
  ...
  ${messages.map(m => `
    <div class="message ${m.role}">
      <div class="role">${m.role === 'user' ? (language === 'ne' ? 'तपाईं' : language === 'hi' ? 'आप' : 'You') : (language === 'ne' ? 'कृषि मित्र' : 'Krishi Mitra')}</div>
      <div class="content">${m.content.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div>
      <div class="time">${format(m.timestamp, 'MMM d, yyyy h:mm a')}</div>
    </div>
  `).join('')}
  <div class="footer">
    <p>${language === 'ne' ? 'CROPIC कृषि मित्र AI सहायकद्वारा उत्पन्न' : language === 'hi' ? 'CROPIC कृषि मित्र AI सहायक द्वारा उत्पन्न' : 'Generated by CROPIC Krishi Mitra AI Assistant'}</p>
  </div>
</body>
</html>`;
